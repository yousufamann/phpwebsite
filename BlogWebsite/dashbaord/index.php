...
<style>
    ...
    * {
    ...
    .stat-card-icon.dots {
        background: linear-gradient(135deg, rgb(47, 13, 240) 0%, rgb(56, 194, 249) 100%);
    }
...
$sql = "SELECT COUNT(*) AS total FROM `post1`";
$user = "SELECT COUNT(*) AS users FROM `users`";
$cat = "SELECT COUNT(*) AS cat FROM `category`";
$feed = "SELECT COUNT(*) AS feed FROM `feedback`";
$comment = "SELECT COUNT(*) AS comment FROM `comment`";
$about = "SELECT COUNT(*) AS about FROM `about`";
$result = mysqli_query($con, $sql);
$result1 = mysqli_query($con, $user);
$result2 = mysqli_query($con, $cat);
$result3 = mysqli_query($con, $feed);
$result4 = mysqli_query($con, $comment);
$result5 = mysqli_query($con, $about);
if ($result) {
    $row = mysqli_fetch_assoc($result);
    $users = mysqli_fetch_assoc($result1);
    $cat = mysqli_fetch_assoc($result2);
    $feedb = mysqli_fetch_assoc($result3);
    $comment = mysqli_fetch_assoc($result4);
    $about = mysqli_fetch_assoc($result5);
}
?>
    ...
    <div class="container">
        ...
        <div class="stats-grid">
            <div class="stat-card">
                ...
                <div class="stat-card-value"><?php echo $users['users']; ?></div>
            </div>
            <div class="stat-card">
                <div class="stat-card-header">
                ...
                <div class="stat-card-value"><?php echo $row['total']; ?></div>
            </div>
            <div class="stat-card">
                <div class="stat-card-header">
                    <span class="stat-card-title">Total Category</span>
                    <div class="stat-card-icon views">
                        <i class="fas fa-th-large"></i>
                    </div>
                </div>
                <div class="stat-card-value"><?php echo $cat['cat']; ?></div>
            </div>
            <div class="stat-card">
                <div class="stat-card-header">
                    <span class="stat-card-title">Total Comments</span>
                    <div class="stat-card-icon dots">
                        <i class="fas fa-comment-dots"></i>
                    </div>
                </div>
                <div class="stat-card-value"><?php echo $comment['comment']; ?></div>
            </div>
            <div class="stat-card">
                <div class="stat-card-header">
                    <span class="stat-card-title">Total Feedback</span>
                    <div class="stat-card-icon comments">
                        <i class="fas fa-comments"></i>
                    </div>
                </div>
                <div class="stat-card-value"><?php echo $feedb['feed']; ?></div>
            </div>
        </div>
        <div style="
    background: white;
...
"
    ...
    ">
        ...
        <?php if (isset($_SESSION['Login_success'])): ?>
            <div style="color: #1565c0; padding:10px; text-align:center;font-size: large;font-weight: bold;">
                <?php

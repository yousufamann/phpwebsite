...
if (isset($_POST['insert'])) {
    $desc = $_POST['desc'];
    $ldesc = $_POST['leftdesc'];
    $rdesc = $_POST['rightdesc'];

    // File details
    $image = $_FILES['image']['name'];
    $limage = $_FILES['leftimage']['name'];
    $rimage = $_FILES['rightimage']['name'];

    $tmp1_image = $_FILES['image']['tmp_name'];
    $tmp2_image = $_FILES['leftimage']['tmp_name'];
    $tmp3_image = $_FILES['rightimage']['tmp_name'];
    ...
    $upload1 = move_uploaded_file($tmp1_image, $upload_path . $image);
    $upload2 = move_uploaded_file($tmp2_image, $upload_path . $limage);
    $upload3 = move_uploaded_file($tmp3_image, $upload_path . $rimage);

    if ($upload1 && $upload2 && $upload3) {
        $sql = "INSERT INTO `about`(`description`, `leftdescription`, `rightdescription`, `image`, `leftimage`, `rightimage`)VALUES ('$desc','$ldesc','$rdesc','$image','$limage','$rimage')";
        
        $query = mysqli_query($con, $sql);

        if ($query) {
            echo "<script>alert('About inserted'); window.location.href='about.php';</script>";
        }
         else {
...
<div style="max-width: 600px; margin: 20px auto; padding: 20px; border-radius: 8px; box-shadow: 0 0 10px rgba(0,0,0,0.1);">
    <form style="display: flex; flex-direction: column; gap: 15px;" method="Post" enctype="multipart/form-data">
        ...
        <div style="display: flex; flex-direction: column; gap: 5px;">
            <label style="font-weight: 500; color: #333;">Left Description</label>
            <input
                type="text"
                style="padding: 10px;border: 1px solid #ddd;border-radius: 4px;font-size: 16px;"
                name="leftdesc" required>
        </div>
        <div style="display: flex; flex-direction: column; gap: 5px;">
            <label style="font-weight: 500; color: #333;">Right Description</label>
            <input
                type="text"
                style="padding: 10px;border: 1px solid #ddd;border-radius: 4px;font-size: 16px;"
                name="rightdesc" required>
        </div>
        <div style="display: flex; flex-direction: column; gap: 5px;">
            <label style="font-weight: 500; color: #333;">Image</label>
            <input
                type="file"
                style="padding: 10px;border: 1px solid #ddd;border-radius: 4px;font-size: 16px;"
                name="image" required>
        ...
        <div style="display: flex; flex-direction: column; gap: 5px;">
            <label style="font-weight: 500; color: #333;">Left Image</label>
            <input
                type="file"
                style="padding: 10px;border: 1px solid #ddd;border-radius: 4px;font-size: 16px;"
                name="leftimage" required>
        </div>
        <div style="display: flex; flex-direction: column; gap: 5px;">
            <label style="font-weight: 500; color: #333;">Right Image</label>
            <input
                type="file"
                style="padding: 10px;border: 1px solid #ddd;border-radius: 4px;font-size: 16px;"
                name="rightimage" required>
        </div>
        <!-- Submit Button -->
        <button
            type="submit"

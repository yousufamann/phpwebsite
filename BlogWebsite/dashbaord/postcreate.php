...
if (isset($_POST['insert'])) {
    ...
    $tmp_image = $_FILES['image']['tmp_name'];
     ...
     if (!is_dir($tmp_image)) {
        mkdir($tmp_image, 0777, true);
    }
    
    if (move_uploaded_file($tmp_image, "uploads-images/". $image)) {
        $sql = "INSERT INTO `post1`(`name`, `description`, `category`, `image`) 
                VALUES ('$name', '$desc', '$cat', '$image')";
        
        $result = mysqli_query($con, $sql);

        if ($result) {
            echo "<script>alert('Post inserted successfully'); window.location.href='post.php';</script>";
        }
         else {
...
<div style="max-width: 600px; margin: 20px auto; padding: 20px; border-radius: 8px; box-shadow: 0 0 10px rgba(0,0,0,0.1);">
    <form style="display: flex; flex-direction: column; gap: 15px;" method="Post" enctype="multipart/form-data">
        <div style="display: flex; flex-direction: column; gap: 5px;">
            <label style="font-weight: 500; color: #333;">Post Name</label>
            <input
                type="text"
                style="padding: 10px;border: 1px solid #ddd;border-radius: 4px;font-size: 16px;"
                name="postname" required>
        </div>
        <div style="display: flex; flex-direction: column; gap: 5px;">
            <label style="font-weight: 500; color: #333;">Post Description</label>
            <input
                type="text"
                style="padding: 10px;border: 1px solid #ddd;border-radius: 4px;font-size: 16px;"
                name="postdesc" required>
        </div>
        <?php
        ...
        ?>
        <div style="display: flex; flex-direction: column; gap: 5px;">
            <label style="font-weight: 500; color: #333;">Category</label>
            <select
                name="catname"
                style="
            padding: 10px;
       ...
       <div style="display: flex; flex-direction: column; gap: 5px;">
            <label style="font-weight: 500; color: #333;">Post Image</label>
            <input
                type="file"
                style="padding: 10px;border: 1px solid #ddd;border-radius: 4px;font-size: 16px;"
                name="image" required>
